...
"""
Script to perform desk cleanup task with URDF-imported objects.

This script demonstrates a desk cleanup task where:
- Objects are imported from URDF files (YCB dataset) and USD files
- Robot grasps and manipulates these objects
- Real-time visualization of GelSight sensor data
- Features 3 clamps and a plate (USD version)

Usage:
    python scripts/desk_cleanup_urdf_demo.py --num_objects 8 --target_object 0
    python scripts/desk_cleanup_urdf_demo.py --headless --num_objects 6
"""
...
parser = argparse.ArgumentParser(description="Desk cleanup with URDF objects and tactile visualization.")
parser.add_argument("--num_envs", type=int, default=1, help="Number of environments to spawn.")
parser.add_argument("--num_objects", type=int, default=8, help="Number of objects on desk (default: 8, max: 8).")
...
# Import URDF-based configuration
from tacex_tasks.desk_cleanup.desk_cleanup_urdf_cfg import DeskCleanupUrdfSceneCfg, create_desk_objects_from_urdf
...
def run_cleanup_demo(
    ...
    """
    Run desk cleanup demonstration with URDF objects and tactile visualization.
    ...
    print("\n" + "=" * 80)
    print("DESK CLEANUP DEMO WITH URDF/USD OBJECTS AND TACTILE VISUALIZATION")
    print("=" * 80)
    ...
    print(f"\n[INFO] Objects imported from URDF and USD files")
    print(f"[INFO] Target object: {target_name} (index {target_object_idx})")
    ...
    print(f"[INFO] Clamps: 3 (clamp_0, clamp_1, clamp_2)")
    print(f"[INFO] Plate: USD version")
...
def main():
    ...
    # Clamp num_objects to max of 8
    num_objects = min(args_cli.num_objects, 8)

    # Create scene configuration
    scene_cfg = DeskCleanupUrdfSceneCfg(num_envs=args_cli.num_envs, env_spacing=2.5)
    scene_cfg.replicate_physics = False

    # Override object_collection with custom number of objects
    scene_cfg.object_collection = create_desk_objects_from_urdf(num_objects=num_objects)

    print(f"\n[CONFIG] Creating desk cleanup scene with {num_objects} URDF/USD objects (3 clamps, plate USD)")
    ...
    # Create scene
    print("\n[INFO] Creating desk cleanup scene with URDF/USD objects (3 clamps + small_KLT)...")
