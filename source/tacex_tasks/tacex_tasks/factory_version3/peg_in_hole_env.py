...
# from isaaclab.utils.math import torch_utils
...
class PegInHoleEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: PegInHoleEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # ========== 新增：触觉数据记录功能 ==========
        self.record_tactile_data = self.cfg.record_tactile_data
        self.tactile_record_interval = self.cfg.record_interval
        self.tactile_record_counter = 0
        self.current_episode = -1  # 初始化为-1，第一个episode会变成0
        self.current_episode_record_dir = None
        
        if self.record_tactile_data and self.cfg.evaluation_mode:
            print("[INFO] Tactile data recording enabled.")
            self.tactile_base_record_dir = os.path.join(os.getcwd(), "tactile_recordings")
            os.makedirs(self.tactile_base_record_dir, exist_ok=True)
            print(f"    > Tactile data will be saved to episode-specific folders under: {self.tactile_base_record_dir}")
            print(f"    > Recording interval: {self.tactile_record_interval} steps")

            self.tactile_left = None
            self.tactile_right = None
            self.tactile_depth_left = None
            self.tactile_depth_right = None
            self.tactile_force_field = None
    ...
    def _setup_scene(self):
        ...
        if self.record_tactile_data and self.cfg.evaluation_mode:  # 只在需要记录时创建相机
    ...
    def _get_peginhole_obs_state_dict(self):
        ...
        if self.cfg_task.tactile["tactile_enabled"] :
            ...
            tactile_depth_left = self.gsmini_left.data.output.get("camera_depth") # num_envs*224*224*1
            tactile_depth_right = self.gsmini_right.data.output.get("camera_depth") 
            tactile_marker_left = self.gsmini_left.data.output.get("marker_motion")  
            tactile_marker_right = self.gsmini_right.data.output.get("marker_motion")  
            # print(1)
            # 2. Extract force field features for observation
            #    tactile_extractor expects (B, H, W, 3) in range [0, 1]
            if self.record_tactile_data and self.cfg.evaluation_mode:
                self.tactile_left = tactile_left
                self.tactile_right = tactile_right
                self.tactile_depth_left = tactile_depth_left
                self.tactile_depth_right = tactile_depth_right
                self.tactile_marker_left = tactile_marker_left
                self.tactile_marker_right = tactile_marker_right

            if tactile_left is not None and tactile_right is not None:
                tactile_force_field_origin, tactile_force_field= self.tactile_extractor(tactile_left.float() / 255.0, tactile_right.float() / 255.0)  # num_envs * 224 * 3
                self.tactile_force_field = tactile_force_field_origin

        if self.cfg_task.tactile["use_contact_forces_as_obs"]:
    ...
    def _get_observations(self):
        ...
        if self.cfg.evaluation_mode and self.cfg.add_obs_noise:  # 仅在play/评估模式下添加噪声
            obs_dict = self._add_observation_noise(obs_dict.copy())
            state_dict = self._add_observation_noise(state_dict.copy())
    ...
    def _add_observation_noise(self, obs_dict):
        """
        在评估模式下，给观测字典中的状态添加噪声，模拟真实传感器的不精确性。
        只在 cfg.evaluation_mode 为 True 时生效。
        
        参数:
            obs_dict (dict): 包含原始观测数据的字典
        
        返回:
            dict: 添加噪声后的观测字典
        """
        if not self.cfg.evaluation_mode:
            return obs_dict
            
        # 位置噪声 - 高斯分布，标准差 1mm
        pos_noise_std = 0.001  # 1毫米
        # 姿态噪声 - 轴角表示，标准差 0.5 度
        rot_noise_std = 0.5 * np.pi / 180.0  # 转换为弧度
        ...
        # 将轴角噪声转换为四元数
        rot_noise_quat = torch_utils.quat_from_angle_axis(
            angle=torch.norm(axis_angle_noise, dim=1),
            axis=axis_angle_noise / (torch.norm(axis_angle_noise, dim=1, keepdim=True) + 1e-8)
        )
        
        # 3. 应用噪声到各个观测项
        # (a) fingertip_pos (指尖位置)
        # if "fingertip_pos" in obs_dict:
        #     obs_dict["fingertip_pos"] = obs_dict["fingertip_pos"] + pos_noise
            
        # # (b) fingertip_pos_rel_fixed (指尖相对位置)
        # if "fingertip_pos_rel_fixed" in obs_dict:
        #     obs_dict["fingertip_pos_rel_fixed"] = obs_dict["fingertip_pos_rel_fixed"] + pos_noise
            
        # (c) held_pos (被夹持工件位置)
        if "held_pos" in obs_dict:
            obs_dict["held_pos"] = obs_dict["held_pos"] + pos_noise
            
        # (d) held_pos_rel_fixed (被夹持工件相对位置)
        if "held_pos_rel_fixed" in obs_dict:
            obs_dict["held_pos_rel_fixed"] = obs_dict["held_pos_rel_fixed"] + pos_noise
            
        # (e) fixed_pos (固定工件位置) - 可选
        if "fixed_pos" in obs_dict:
            obs_dict["fixed_pos"] = obs_dict["fixed_pos"] + pos_noise
            
        # (f) 所有姿态四元数 - 需要右乘噪声四元数
        quat_fields = ["fingertip_quat", "fingertip_quat_rel_fixed", "held_quat", "fixed_quat"]
        for field in quat_fields:
            if field in obs_dict and obs_dict[field] is not None:
                # 确保四元数是单位四元数
                quat = torch_utils.normalize(obs_dict[field])
                # 添加噪声 (右乘)
                obs_dict[field] = torch_utils.quat_mul(quat, rot_noise_quat)
                # 重新归一化
                obs_dict[field] = torch_utils.normalize(obs_dict[field])
                
        # # (g) 速度观测也可以添加噪声（通常需要较小的标准差）
        # vel_noise_std = 0.005  # 5mm/s
        ...
        # vel_fields = ["ee_linvel", "ee_angvel"]
        # for field in vel_fields:
        #     if field in obs_dict:
        #         obs_dict[field] = obs_dict[field] + vel_noise
                
        # # 可选：为触觉力场特征添加噪声
        # if "tactile_force_field" in obs_dict and obs_dict["tactile_force_field"] is not None:
        #     force_noise_std = 0.05  # 5% 的力测量噪声
        #     force_noise = torch.normal(
        #         mean=0.0,
        #         std=force_noise_std,
        #         size=obs_dict["tactile_force_field"].shape,
        #         device=self.device
        #     )
        #     obs_dict["tactile_force_field"] = obs_dict["tactile_force_field"] + force_noise
    ...
    def _get_dones(self):
        ...
        self._compute_intermediate_values(dt=self.physics_dt)
        # 在这个环境中，唯一的结束条件就是超时。
        time_out = self.episode_length_buf >= self.cfg.max_episode_length - 1
    ...
    def _get_rewards(self):
        ...
        if self.cfg.evaluation_mode:
            ...
            if self.record_tactile_data :
                curr_successes = self._get_curr_successes(self.cfg_task.success_threshold)
                if not self.episode_success and curr_successes[0]:  # 只看 env 0
                    self.episode_success = True
                    self.episode_success_step = int(self.episode_length_buf[0].item())
                self.tactile_record_counter += 1
                if self.tactile_record_counter % self.tactile_record_interval == 0:
                    current_step = int(self.episode_length_buf[0].item())
                    self._record_tactile_data(current_step)
    ...
    def _reset_idx(self, env_ids):
        ...
        if self.record_tactile_data and self.cfg.evaluation_mode:
            ...
            # 创建当前episode的记录目录
            self.current_episode_record_dir = os.path.join(
                self.tactile_base_record_dir, 
                f"episode_{self.current_episode:04d}"
            )
            os.makedirs(self.current_episode_record_dir, exist_ok=True)
            # 创建子目录用于不同类型的触觉数据
            os.makedirs(os.path.join(self.current_episode_record_dir, "rgb"), exist_ok=True)
            os.makedirs(os.path.join(self.current_episode_record_dir, "depth"), exist_ok=True)
            os.makedirs(os.path.join(self.current_episode_record_dir, "force_field"), exist_ok=True)
            # os.makedirs(os.path.join(self.current_episode_record_dir, "camera"), exist_ok=True)
            os.makedirs(os.path.join(self.current_episode_record_dir, "pose_data"), exist_ok=True)     
            os.makedirs(os.path.join(self.current_episode_record_dir, "marker_motion"), exist_ok=True)     
            print(f"[Tactile Recording] Starting new episode recording: {self.current_episode_record_dir}")
    ...
    def _record_tactile_data(self, step_count):

        """记录触觉传感器数据，使用已计算好的数据，避免重复推理"""
        if not self.record_tactile_data or not self.cfg_task.tactile["tactile_enabled"]:
            return
        if self.current_episode_record_dir is None:
            print("[Tactile Recording] No episode directory set. Skipping recording.")
            return
        ...
        if self.tactile_left is None or self.tactile_right is None:
            print("[Tactile Recording] No tactile data available for recording")
            return
        
        # 1. 保存RGB图像
        if self.tactile_left is not None and self.tactile_right is not None:
            # 左传感器RGB
            tactile_left_np = (self.tactile_left[env_id].cpu().numpy() * 255).astype(np.uint8)
            tactile_left_bgr = cv2.cvtColor(tactile_left_np, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(self.current_episode_record_dir, "rgb", f"left_{step_count:06d}.png"), tactile_left_bgr)
            
            # 右传感器RGB
            tactile_right_np = (self.tactile_right[env_id].cpu().numpy() * 255).astype(np.uint8)
            tactile_right_bgr = cv2.cvtColor(tactile_right_np, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(self.current_episode_record_dir, "rgb", f"right_{step_count:06d}.png"), tactile_right_bgr)
        
        # 2. 保存深度图像
        if self.tactile_depth_left is not None and self.tactile_depth_right is not None:
            # 左传感器深度
            depth_left_np = self.tactile_depth_left[env_id].cpu().numpy()
            # 归一化深度图像以便可视化
            depth_left_vis = ((depth_left_np - depth_left_np.min()) / 
                            (depth_left_np.max() - depth_left_np.min()) * 255).astype(np.uint8)
            cv2.imwrite(os.path.join(self.current_episode_record_dir, "depth", f"left_{step_count:06d}.png"), depth_left_vis.squeeze())
            
            # 右传感器深度
            depth_right_np = self.tactile_depth_right[env_id].cpu().numpy()
            depth_right_vis = ((depth_right_np - depth_right_np.min()) / 
                            (depth_right_np.max() - depth_right_np.min()) * 255).astype(np.uint8)
            cv2.imwrite(os.path.join(self.current_episode_record_dir, "depth", f"right_{step_count:06d}.png"), depth_right_vis.squeeze())
        
        # 3. 保存力场特征 (作为numpy数组)
        # 在 _record_tactile_data 中，替换 force field 保存部分
        if self.tactile_marker_left  is not None and self.tactile_marker_right is not None:
            marker_data_left = self.tactile_marker_left[env_id].cpu().numpy()  # Shape: [2, 99, 2]
            marker_data_right = self.tactile_marker_right[env_id].cpu().numpy()  # Shape: [2, 99, 2]
            marker_motion_dir = os.path.join(self.current_episode_record_dir, "marker_motion")
            # 创建要保存的字典
            # 数据结构: [initial_pos, current_pos] -> [{'x': val, 'y': val}, ...]
            marker_dict_left = {
                'step': step_count,
                'env_id': env_id,
                'data': {
                    'initial_positions': marker_data_left[0].tolist(),  # Shape: [99, 2]
                    'current_positions': marker_data_left[1].tolist(),  # Shape: [99, 2]
                    # 可选：计算并保存位移
                    # 'displacements': (marker_data_left[1] - marker_data_left[0]).tolist() # Shape: [99, 2]
                }
            }
            
            # 保存为JSON文件
            marker_file_left = os.path.join(marker_motion_dir, f"left_markers_{step_count:06d}.json")
            with open(marker_file_left, 'w') as f:
                json.dump(marker_dict_left, f, indent=2)
        
            marker_dict_right = {
                'step': step_count,
                'env_id': env_id,
                'data': {
                    'initial_positions': marker_data_right[0].tolist(),  # Shape: [99, 2]
                    'current_positions': marker_data_right[1].tolist(),  # Shape: [99, 2]
                    # 可选：计算并保存位移
                    # 'displacements': (marker_data_right[1] - marker_data_right[0]).tolist() # Shape: [99, 2]
                }
            }
            marker_file_right = os.path.join(marker_motion_dir, f"right_markers_{step_count:06d}.json")
            with open(marker_file_right, 'w') as f:
                json.dump(marker_dict_right, f, indent=2)


        if self.tactile_force_field is not None:
            # 提取 env_id 对应的 force field 数据
            normal = self.tactile_force_field['normal'][env_id]  # [1, 224, 224]
            shear = self.tactile_force_field['shear'][env_id]    # [2, 224, 224]
            
            # 将数据从 (C, H, W) 转换为 (H, W, C) 方便处理
            force_field = torch.cat([shear, normal], dim=0).permute(1, 2, 0).cpu().numpy()  # [224, 224, 3]
            ...
            # 计算下采样后的网格点坐标
            x = np.arange(0, W, step)  # [0, 8, 16, ..., 216]
            y = np.arange(0, H, step)  # [0, 8, 16, ..., 216]
            X, Y = np.meshgrid(x, y)   # X, Y: [28, 28]
            
            # 提取下采样后的切向力 (U, V) 和法向力 (C)
            U = -force_field[Y, X, 0]  # [28, 28]
            V = -force_field[Y, X, 1]  # [28, 28]
            C = force_field[Y, X, 2]   # [28, 28], 注意这里保留了原始符号，不再取绝对值
            
            # 将所有数据打包成一个字典
            force_field_data = {
                'step': step_count,
                'grid_x': X.tolist(),           # 网格X坐标 [28,28]
                'grid_y': Y.tolist(),           # 网格Y坐标 [28,28]
                'shear_x': U.tolist(),          # 切向力X分量 [28,28]
                'shear_y': V.tolist(),          # 切向力Y分量 [28,28]
                'normal_z': C.tolist()          # 法向力Z分量 [28,28]
            }
            
            # 保存为JSON文件
            force_field_path = os.path.join(self.current_episode_record_dir, "force_field", f"data_{step_count:06d}.json")
            os.makedirs(os.path.dirname(force_field_path), exist_ok=True)
            with open(force_field_path, 'w') as f:
                json.dump(force_field_data, f, indent=2)
        
        # cam_data = self._tiled_camera.data.output["rgb"] / 255.0

        # if cam_data is not None:
        #     cam_img = cam_data[env_id].cpu().numpy()  # [H, W, 3], uint8 or float?
        #     if cam_img.dtype != np.uint8:
        #         cam_img = (cam_img * 255).astype(np.uint8)
        #     cam_bgr = cv2.cvtColor(cam_img, cv2.COLOR_RGB2BGR)
        #     cv2.imwrite(
        #         os.path.join(self.current_episode_record_dir, "camera", f"frame_{step_count:06d}.png"),
        #         cam_bgr
        #     )
            # ========== 新增：记录 env_0 的工件位姿信息 ==========
        # 获取当前时间步的工件位姿
        ...
        # 提取 env_0 的数据并转换为 numpy 便于保存
        held_pos_np = held_base_pos[env_id].cpu().numpy()       # [3]
        held_quat_np = held_base_quat[env_id].cpu().numpy()     # [4]
        target_pos_np = target_held_base_pos[env_id].cpu().numpy()  # [3]
        target_quat_np = target_held_base_quat[env_id].cpu().numpy() # [4]

        # 创建 pose_data 字典
        pose_data = {
            'step': step_count,
            'held_base_pos': held_pos_np.tolist(),        # 转为 list 便于 json 序列化
            'held_base_quat': held_quat_np.tolist(),
            'target_held_base_pos': target_pos_np.tolist(),
            'target_held_base_quat': target_quat_np.tolist()
        }

        # 保存 pose_data 到 JSON 文件
        pose_file_path = os.path.join(self.current_episode_record_dir, "pose_data", f"pose_{step_count:06d}.json")
        os.makedirs(os.path.dirname(pose_file_path), exist_ok=True)  # 确保 pose_data 目录存在
        with open(pose_file_path, 'w') as f:
            json.dump(pose_data, f, indent=2)  # indent 使 JSON 文件格式更易读
        # 4. 保存元数据
        if torch.any(self.reset_buf):
            episode_meta = {
                "episode_number": self.current_episode,
                "success": self.episode_success,
                "success_step": self.episode_success_step,
                "total_steps": int(self.episode_length_buf[env_id].item()),
                "record_interval": self.tactile_record_interval,
            }
            
        print(f"[Tactile Recording] Episode {self.current_episode} | Saved tactile data at step {step_count}")
        
        # except Exception as e:
        #     print(f"[Tactile Recording] Episode {self.current_episode} | Error saving tactile data: {e}")
